...
const Values = ({ translateX }: ValuesProps) => {
  ...
  const change = useDerivedValue(
    () => `${candle.value.close - candle.value.open < 0 ? diff.value.substring(0, 5) : diff.value.substring(0, 4)}%`,
  );
  const white = useDerivedValue(() => (candle.value.close - candle.value.open > 0 ? '#fff' : '#fff'));
  const color = useDerivedValue(() => (candle.value.close - candle.value.open > 0 ? '#4AFA9A' : '#E33F64'));
  const date = useDerivedValue(() => formatDatetime(candle.value.date));
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.table}>
        <View style={styles.column}>
          <Row label="Open" value={open} color={color} />
          <Row label="Close" value={close} color={color} />
        </View>
        <View style={styles.separator} />
        <View style={styles.column}>
          {/* <Row label="High" value={high} color={color} />
          <Row label="Low" value={low} color={color} /> */}
